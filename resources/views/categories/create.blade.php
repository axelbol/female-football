<x-layouts.app :title="__('Create Category')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ __('Create Category') }}</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Add a new category to organize your blog posts') }}</p>
            </div>
                ...
                <form action="{{ route('categories.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="color"
                            ...
                            <option value="emerald" {{ old('color', 'emerald') === 'emerald' ? 'selected' : '' }}>{{ __('Emerald') }}</option>
                            ...
                            <option value="purple" {{ old('color') === 'purple' ? 'selected' : '' }}>{{ __('Purple') }}</option>
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  ...
                                  placeholder="{{ __('Optional description for this category') }}">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
